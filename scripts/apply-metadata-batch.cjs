...
const batch = [
  ...
  ["霓虹甜心", "马赛克", "2012", "摇滚", "中文"],
  ["你啊你啊", "魏如萱", "2016", "流行", "中文"],
  ["你还要我怎样", "薛之谦", "2013", "流行", "中文"],
  ["日落大道", "梁博", "2015", "摇滚", "中文"],
  ["说了再见", "周杰伦", "2010", "流行", "中文"],
  ["天地龙鳞", "王力宏", "2021", "流行", "中文"],
  ["我还想她", "林俊杰", "2008", "流行", "中文"],
  ["无底洞", "蔡健雅", "2003", "流行", "中文"],
  ["心中的日月", "王力宏", "2004", "流行", "中文"],
  ["行走的鱼", "徐佳莹", "2018", "流行", "中文"],
  ["一千年以后", "林俊杰", "2005", "流行", "中文"],
  ["Forever Love", "王力宏", "2004", "流行", "英文"],
  ["Yellow", "Coldplay", "2000", "摇滚", "英文"],
  ["爱的飞行日记", "周杰伦 / 杨瑞代", "2010", "流行", "中文"],
  ["爱在西元前", "周杰伦", "2001", "流行", "中文"],
  ["被驯服的象", "蔡健雅", "2013", "流行", "中文"],
  ["变废为宝", "薛之谦", "2020", "流行", "中文"],
  ["不为谁而作的歌", "林俊杰", "2015", "流行", "中文"],
  ["不值得", "梦飞船", "1999", "流行", "中文"],
  ["程艾影", "赵雷", "2016", "民谣", "中文"],
  ["匆匆那年", "王菲", "2014", "流行", "中文"],
  ["第一个清晨", "王力宏", "2005", "流行", "中文"],
  ["发如雪", "周杰伦", "2005", "中国风", "中文"],
  ["反方向的钟", "周杰伦", "2000", "流行", "中文"],
  ["关键词", "林俊杰", "2015", "流行", "中文"],
  ["黑色幽默", "周杰伦", "2000", "流行", "中文"],
  ["黑夜中", "梁博", "2015", "摇滚", "中文"],
  ["哪里都是你", "队长", "2020", "流行", "中文"],
  ["秋意浓", "张学友", "1993", "流行", "中文"],
  ["听见下雨的声音", "魏如昀", "2013", "流行", "中文"],
  ["突然好想你", "五月天", "2008", "流行", "中文"],
  ["夜空中最亮的星", "逃跑计划", "2011", "摇滚", "中文"],
  ["一直很安静", "阿桑", "2005", "流行", "中文"],
  ["雨下一整晚", "周杰伦", "2010", "中国风", "中文"],
  ["暗恋", "张智成", "2009", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["不染", "毛不易", "2018", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["曹操", "林俊杰", "2006", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["单车", "陈奕迅", "2001", "流行", "粤语", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["红豆", "王菲", "1998", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["画", "赵雷", "2011", "民谣", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["理想", "赵雷", "2014", "民谣", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["内疚", "陈奕迅", "2011", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["瘦子", "陶喆", "2013", "R&B", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["瞬", "郑润泽", "2021", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["天后", "陈势安", "2009", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["退后", "周杰伦", "2006", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["危险", "徐佳莹", "2009", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["小宇", "张震岳", "2007", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["演员", "薛之谦", "2015", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["阴天", "莫文蔚", "1999", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
  ["知足", "五月天", "2005", "流行", "中文", "短歌名可能重名；按常见版本默认归入，待歌手本人确认"],
].map(([title, originalArtist, releaseYear, genre, language, confirmationNote = ""]) => ({
  singerId: "",
  title,
  originalArtist,
  releaseYear,
  ...
  for (const file of fs.readdirSync(SONGLISTS_DIR).filter((name) => name.endsWith("_南音_歌单.json"))) {
    ...
    for (const songs of Object.values(doc.songs || {}).filter(Array.isArray)) {
      for (const song of songs) {
        ...
        if (metadata.confirmationNote) {
          song.confirmationNote = metadata.confirmationNote;
        } else {
          delete song.confirmationNote;
        }
        touched = true;
        touchedSongs += 1;
      }
